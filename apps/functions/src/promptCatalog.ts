...
function buildInterviewStateInstruction(
  ...
  const clothingHint = (kind === 'clothing' && clothingTypeFromMeta && modeLocked)
    ...
    : (kind === 'clothing' && clothingTypeFromMeta)
      ...
      : '';
  if (turn === 0) {
    return `INTERVIEW STATE: This is the FIRST turn (turn 0). Greet enthusiastically and acknowledge the idea. Your DEFAULT first question is about ${interviewFirstQuestionFocus(kind, metadata)}. ${interviewTurnFocus(kind, turn)} CRITICAL — be smart, never redundant or interrogating: if the user's first message ALREADY answers that default question (very common when they tapped a detailed preset, or named a specific concept/type/theme/style), do NOT ask them to re-pick or restate it. Confirm in one line what you understood, then EITHER ask the single most useful UNANSWERED question, OR — when the only thing left is a low-stakes choice (e.g. which single item to build first from a themed set/pack) — propose a sensible default ("I'll start with X — the centerpiece") and let them confirm or tweak it, instead of quizzing them. Never make the user repeat information they already gave. If quickReplies help, include at most 3 and make one "Decide for me".${clothingHint}`;
  }
  if (turn >= maxTurns) {
