...
function buildEntityShapeGuidance(input: DisasterPromptInput): string[] {
  ...
  if (input.assetEntries && input.assetEntries.length > 0) {
    ...
    const luaTemplate = [
      ...
      `local waveCounter = 0`,
      ``,
      `-- ===== TEMPLATE CACHE — load each mesh ONCE, clone for every spawn.`,
      `-- AssetService:CreateMeshPartAsync is a CDN fetch (~50-300ms each); calling`,
      `-- it 30 times per wave hitches the server thread. We cache one off-tree`,
      `-- MeshPart per mesh id and Clone() for every spawn — 1-2 ms per spawn.`,
      `local templateCache = {}`,
      `local function getTemplate(meshId)`,
      `\tif templateCache[meshId] then return templateCache[meshId] end`,
      ...
      `\t\twarn("[DisasterSpawner] CreateMeshPartAsync failed for " .. tostring(meshId) .. ": " .. tostring(fresh))`,
      `\t\treturn nil`,
      `\tend`,
      `\tfresh.Anchored = true`,
      `\tfresh.Parent = nil   -- off-tree archive`,
      `\ttemplateCache[meshId] = fresh`,
      `\treturn fresh`,
      `end`,
      ``,
      ...
      `\t\tif hrp then`,
      `\t\t\treturn hrp.Position + Vector3.new(math.random(-40, 40), 140, math.random(-40, 40))`,
      `\t\tend`,
      `\tend`,
      `\treturn Vector3.new(math.random(-50, 50), 140, math.random(-50, 50))`,
      `end`,
      ``,
      ...
      `\tlocal template = getTemplate(meshId)`,
      `\tif not template then return nil end`,
      `\tlocal mp = template:Clone()  -- instant; no CDN fetch`,
      ...
      `\t-- SLOW-FALL: add an upward VectorForce that counteracts 65% of gravity`,
      `\t-- for 6 seconds. Effective gravity ≈ 0.35 × 196.2 = 68 studs/s² →`,
      `\t-- from spawn height 140 the mesh takes ~2.0s to fall instead of ~1.2s.`,
      `\t-- Long enough that the warning marker is actually useful + the splash`,
      `\t-- moment reads cinematically.`,
      `\tlocal supportAtt = Instance.new("Attachment")`,
      `\tsupportAtt.Parent = mp`,
      `\tlocal slowFall = Instance.new("VectorForce")`,
      `\tslowFall.Attachment0 = supportAtt`,
      `\tslowFall.RelativeTo = Enum.ActuatorRelativeTo.World`,
      `\tslowFall.Force = Vector3.new(0, mp.AssemblyMass * 196.2 * 0.65, 0)`,
      `\tslowFall.Parent = mp`,
      `\tDebris:AddItem(slowFall, 6)`,
      `\tDebris:AddItem(supportAtt, 6.1)`,
