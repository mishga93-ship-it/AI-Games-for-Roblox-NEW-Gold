...
### 🧹 [Cleanup — удалён осиротевший FakeLimitedRecipeCard] (2026-06-05, сессия 426)
- **Задача** (пользователь): подчистить мёртвый «Variant 3» фичи Fake Headless & Korblox.
- **Сделано**: удалён `apps/ios/AIGoldRoblox/Features/FakeLimited/FakeLimitedRecipeCard.swift` (был `#if false`, вьюха `FakeLimitedRecipeCard` не использовалась) + пустая папка; из `project.pbxproj` сняты 5 записей (PBXBuildFile/FileReference/Group `FakeLimited`/parent-ref/SourcesBuildPhase). Фича работает через **Avatar Glow-Up** chat-flow; legacy-путь (`ChatStore.FakeLimitedRecipe`/`handleFakeLimitedUserMessage`/`/api/fake-limited/recipe`) как fallback **не тронут**.
- **Проверка**: `plutil -lint` → OK; `xcodebuild -list` → проект парсится (target/схема/конфиги целы; полный build не гонял — Xcode открыт §0.7, и файл был `#if false` → удаление инертно).
- **Git**: `project.pbxproj` ` M`, файл ` D`; не закоммичено (коммит/деплой — отдельной командой). Детали — `cursor/changelog-426.md`.
- **Результат**: выполнено.

### 🛻 [Brainrot Vehicle Generator — Variant 3, Phase A] Слот-генератор абсурдного транспорта «кузов = объект» (2026-06-05, сессия 425)
- **Задача** (пользователь): генератор «максимально абсурдного TikTok-Brainrot транспорта» по слотам (body/head/wheels/engine/effects/sound), drivable. Выбор (AskUserQuestion): **гибрид ассетов · только транспорт · «кузов = объект»**. План: `.claude/plans/binary-juggling-spring.md`.
- **Ключевой инсайт**: «кузов = объект» уже на 90% существует — mesh-body ветка (`vehicleMeshAssetId`→`VehicleMeshBody`, welded к `ChassisRoot`) + `getOrCreateDisasterMesh()` (Meshy+кэш под те же ключевые слова). Фича = соединить слот-роутер с процедурным рендером.
- **Сделано (Phase A, БЕЗ платных вызовов)**:
  - [vehicleModular.types.ts](apps/functions/src/vehicleModular.types.ts): `BrainrotSlots` + enum'ы wheels/engine/effects/sound + `BRAINROT_PROCEDURAL_BODIES/HEADS`.
  - [vehicleModular.router.ts](apps/functions/src/vehicleModular.router.ts): `isBrainrotIntent()` + `routeBrainrotSlots()` (Gemini→slot-JSON, sanitize + keyword-fallback).
  - [vehicleModular.library.ts](apps/functions/src/vehicleModular.library.ts): `buildBrainrotLuaBlock()` — 8 голов-топперов + engine VFX (jet/rocket/nuclear/propeller) + wheel VFX (hover/rocket) + effects (rainbow/fire/lightning/confetti/sparkles/smoke); всё через `mount()` → трансмиссию не трогает.
  - [robloxWorker.ts](apps/functions/src/robloxWorker.ts): `addNoveltyBodyShell()` (banana/toilet/fridge/pizza/shopping_cart/duck из примитивов, welded к `ChassisRoot`); гейт `metadata.vehicleNoveltyBodyType` в ветке `!useTemplateEmbed` → drivable.
  - [vehicleModular.builder.ts](apps/functions/src/vehicleModular.builder.ts): резолв слотов → `brainrotMetadata`; force процедурного пути (пустой template-filename).
  - [index.ts](apps/functions/src/index.ts): merge `brainrotMetadata` + **skip платного `generate_vehicle_mesh` через флаг `vehicleBrainrot`** (иначе пустой filename → запустился бы Meshy).
- **Проверка**: `npm run build --workspace apps/functions` ✅ tsc clean. Lua-смоук `node scripts/smoke-brainrot.mjs` ✅ **27/27 ok, 0 fail** (luau.compile всей матрицы слотов; прогнал пользователь).
- **Не сделано**: commit/deploy (по правилам — после отдельной команды пользователя); визуал в Studio.
- **Дальше (Phase B/C)**: Tier-2 (`getOrCreateDisasterMesh`→`vehicleMeshAssetId` для не-Tier-1 body/head); реальная геометрия колёс monster/tank/tiny; проверенные audio assetId для sound-слота; iOS chat-flow триггер (`contentSubcategory`).

### 📖 [Story Game — нарративное приключение] Катсцены + ветвление + выборы + 3 концовки + диалоги + головоломка (2026-06-04, сессия 421)
- **Задача** (пользователь): доработать Story Game — катсцены, ветвление сюжета, выборы с последствиями, несколько концовок, NPC с диалогами, головоломки; проверить, что ассеты-пресеты Roblox нормально отображаются (≤10/игра, по тематике).
- **Сделано** ([gameTemplates.ts](apps/functions/src/gameTemplates.ts) `buildStoryGameScript` переписан, +`STORY_DEFS` 4 темы):
  - **Катсцены**: интро (скриптовая камера по waypoint'ам + letterbox + typewriter) + финальная; `Scriptable`+`TweenService`+`CFrame.lookAt`.
  - **NPC-диалоги**: 3 NPC (голова-портрет+имя+Talk), typewriter-панель со speaker, фокус камеры + блюр.
  - **Ветвление/выборы с последствиями**: 2 кнопки выбора на акт → копят `karma`; в актах 2–3 «recall»-эхо прошлого выбора.
  - **Несколько концовок**: `pickEnding(karma)` → 3 концовки по порогам, пишется в `leaderstats.Ending`.
  - **Головоломка**: 3 руны (ProximityPrompt) в заданном по теме порядке → открывают физический барьер ко 2-му акту (failsafe 150с от застревания).
- **Theme-ассеты story** ([themeAssetPacks.ts](apps/functions/src/data/themeAssetPacks.ts)): раскладка ВДОЛЬ коридора (`corridorLua`, x=±16, z 30..350) вместо кольца у спавна; cap 8 (≤10). Остальные жанры не тронуты.
- **Проверка**: tsc clean; `node smoke-story-adventure.mjs` ✅ — 4 темы компилируются (Lune), 6 фич у каждой, scatter-коридор ок.
- **ВНЕШНИЙ БЛОКЕР**: 3 Story-пресета без каталог-ассетов → нужны ссылки от пользователя: **The Last Night Guard**, **Orangutini's Kingdom**, **The Haunted Sleepover** (нет matching-пака в `THEME_ASSET_PACKS`).
- **Не сделано**: commit/deploy (в gameTemplates.ts незакоммиченные правки параллельной сессии — нужен private-index commit по команде); визуал не проверен в Studio.

### 🎮 [Генерация игр — узнаваемость по пресетам + QA] Все 9 жанров: per-preset visual-spec + авто-QA (2026-06-03, сессия 414)
- **Задача** (пользователь): жанры Roleplay/Town, Tower Defense, Racing, Parkour, Story Game, Mini-games Hub, Survival, Fighting, Custom — при генерации по пресетам результат не похож на пресет; поднять узнаваемость (возможно 3D-меши), добавить QA-проверку с авто-перегенерацией. Решение пользователя (AskUserQuestion): «узнаваемость + QA, без меша», «пилот на 1 жанре потом все».
- **Root cause**: 9 билдеров в [gameTemplates.ts](apps/functions/src/gameTemplates.ts) поддерживали только ~4 фикс-енума `mapTheme`; LLM-GDD почти никогда не отдавал `mapTheme` → в [index.ts](apps/functions/src/index.ts) `gddEnum(...,DEFAULT)` всегда брал default → все пресеты жанра = один мир. Текст пресета до билдера не доходил.
- **Решение** (детерминированный «hybrid skeleton», по памяти проекта):
  - Новый [gameThemeSpec.ts](apps/functions/src/gameThemeSpec.ts): `deriveGameVisualSpec(genre,brief,title)` — из брифа выводит **setting** (town/school/island/lab/kingdom → 6 зданий+работ) × **vibe** (brainrot/money/monster/inferno/spy/tropical/night/hero/pets/lab/neutral → палитра+NPC-реплики+hero-keyword+iconic-здание). + `reviewGameQuality` (part-count градуир., leaderstats, signage, тематические вывески для town-like, palette-applied) + `healGameVisualSpec` (self-heal). + `rgbLua` (единый цвет-форматтер для билдера и QA).
  - `GameTemplateParams.gameVisualSpec?` прошит во все 9 билдеров. **Roleplay** (пилот) — полная прошивка: палитра + 6 тематических зданий + работы + NPC-реплики + подпись плазы. **Остальные 8** — палитра-override при `vibe!==neutral` (neutral → enum-тема билдера, без регрессии; Survival — безопасный merge цветовых полей). Отсутствие спека → оригинальное поведение (no regression).
  - [index.ts](apps/functions/src/index.ts): `buildPlayableWithQa(genre,params,brief,title)` — derive→build→QA→если rejected: heal+rebuild, вернуть лучший. 9 веток subcategory переведены на неё. Бриф = title + текст пресета (latestUserIntent) + GDD.
- **Проверка**: `npm run build --workspace apps/functions` ✅ tsc clean. Смоук `smoke-game-recognizability.mjs`: все 9 жанров — palette applied, QA passed (88–100), no-spec регрессия цела; roleplay — 4 пресета → 4 разных набора зданий/палитры/работ/NPC. Emitted Lua проверен вручную (валиден).
- **Атмосфера (фидбэк по TD «99 Nights» — солнечный луг вместо ночи)**: палитра меняла только землю, а небо/деревья были захардкожены под день. Добавлен `GameAtmosphere` в спек (mood night/dusk/day + clockTime/яркость/туман + тип/цвет деревьев) + `atmosphereOptsLua`, прошит в 8 `setupAtmosphere`-билдеров; TD дополнительно — хвойный лес + **костёр** для night-вайба. Коммит `1bb91b1`, задеплоено.
- **Контент (фидбэк по racing «Lava City» — нет лавы/города)**: палитра+небо ≠ контент. Racing-декор был захардкожен (зелёные сосны). Добавлен vibe-driven контент: inferno → лавовые озёра + вулканические башни города + мёртвые деревья; urban → башни; tropical → пальмы. Коммит `363e6f3`, задеплоено.
- **Контент-проход Roleplay/Town (по полной спеке 70 пресетов; выбор: roleplay первым + меши)**: `GameCharacter`-ростеры по вайбам — Brainrot Town спавнит Tralalero/Bombardiro/Skibidi NPC с реальными мем-лицами (rbxthumb-декали), MrBeast→Host/Challenger/Prize Master, 99 Nights→Survivor/Scout/Owl Keeper и т.д.; + hero-пьедестал на плазе с лицом сигнатурного персонажа. Коммит `ee18df4`, задеплоено.
- **Сигнатурные объекты Roleplay (по описаниям 70-пресет-спеки)**: на каждую тему свои объекты у плазы — Money Vault+Challenge (MrBeast), костёр+палатка (99 Nights), кладбище (Monster), антенна+туннель (Spy), тренир.арена (Superhero), яйцо дракона+Pet Park (Pets), банановый ларёк+тики (Bananita), контейнеры (Lab), мем-постаменты (Brainrot). Фикс детекции: Giant Pet World→pets, Bananita Island→tropical. Коммит `73fa00e`, задеплоено. **Roleplay/Town free-контент готов целиком.**
- **Tower Defense враги (контент-проход, выбор «давай 2»)**: `enemies`-ростеры по вайбам — Brainrot Defense→Tralalero/Bombardiro/Tung Tung с реальными мем-лицами, 99 Nights→Wolf/Night Stalker/Shadow, FNAF/Titan/Monster→Animatronic/Mutant/Titan, Bananita→тропич.; босс = последний в ростере; враг рендерит лицо-декаль через rbxthumb. Коммит `8622a97`, задеплоено.
- **Racing/Story/Mini-games/Custom контент**: `themedSpawnBillboardLua` — баннер с лицом сигнатурного персонажа + название над спавном (story/minigame/custom/racing); racing-декор расширен на night/monster/pets (лес+костёр). Parkour пока только палитра+небо (нет SpawnLocation). Коммит `10371e7`, задеплоено.
- **Survival/Fighting враги**: survival-преследователи берут цвет+имя+лицо из `spec.enemies` (99 Nights→волки, Prototype→Toy Mutant/Glitch/Prototype, Bananita→тропич., Monster School→Animatronic/Mutant/Titan); fighting (PvP-ринг) → тематический landmark. Коммит `dfa8ba9`, задеплоено. **Контент-проход: 8/9 жанров готовы (Roleplay/TD/Racing/Story/Minigames/Custom/Survival/Fighting); Parkour — палитра+небо, landmark next.**
- **🏁 Parkour landmark → ВСЕ 9 ЖАНРОВ ЗАКРЫТЫ**: parkour ParkourSpawn получил тематический landmark. Коммит `1ebad6d`. **Free-контент-проход завершён: каждый из 9 рантайм-жанров несёт per-preset тематику (палитра+небо+персонажи/враги/объекты/landmark) + QA-авто-перегенерация.** Опционально дальше: Meshy 3D hero-меш (платный, за QA).
- **Премиум-3D (анализ + proof)**: каталог/композит-ассеты узнаваемее Meshy для мем-тем (+ Meshy-загрузчик промодерён → невидим). Извлёк проверенные композитные 3D-мем-билдеры (Tralalero/Skibidi/Bombardiro/Sigma) в `meme3dPreludeLua()`; hero-центр roleplay = реальный 3D (плоская декаль убрана). Коммит `19af02e`. **⚠️ Нужна проверка визуала в Studio (сам не вижу) → потом раскатка 3D на NPC/врагов/жанры + каталог-меши для не-мем тем.**
- **Декластер мира (фидбэк «понамешено»)**: убрана trending-витрина из всех 9 жанров; билборды `AlwaysOnTop=false`+`MaxDistance` (по devforum); вывески зданий → SurfaceGui на стене; 3D-фигуры без spin + крупнее; NPC-лица декластерены. Коммит `8797999`. **Стейдж: NPC→3D-фигуры, enterable-здания.**
- **Заходимые здания + R15-NPC**: `buildHouse` → полый дом (4 стены + дверной проём + пол + крыша + лампа); NPC → реальные R15-гуманоиды (`humanoidNpcPreludeLua`, заякорены, имя+лицо+Talk) вместо блоков. Коммит `00ead40`, задеплоено. Нужна проверка визуала в Studio.
- **NPC=3D-существа + RP-доводка**: мем-NPC спавнятся как реальные 3D-фигуры (акула/крокодил), не баннеры; деревья в углы (не в дома); мебель в дома; работы Police Officer/Doctor/Chef + Hospital/Diner/Police. Коммит `ec5dfff`. Стейдж: машины/питомцы/кастомизация дома.
- **Питомцы (PivotTo-фикс) + ездящие машины**: питомец следует целиком (все части anchored, движение `m:PivotTo` — голова больше не отстаёт); машины driveable через `VehicleSeat` (anchor пока пусто → unanchor+`SetNetworkOwner` при посадке). Коммит `3841440`, задеплоено.
- **4 активности Roleplay (фидбэк «все сразу»)**: (1) **живые работы** — clock-in на job-паде → доставка в Delivery Depot за 2× оплату (движение по городу вместо idle-кнопки); (2) **свой дом** — пригород из 3 plot'ов, Claim → Recolor стен + Add Furniture (стол/диван/лампа/растение), 1 на игрока; (3) **монеты-квест** — 12 светящихся монет (r=70, bob, респаун 12с) +$35 за сбор + QuestBoard; (4) **автосалон** ($500 → личная driveable-машина, `buildCar` теперь `return model`) + **банкомат** ($150/60с кулдаун). Всё server-side через `RpEvent` toast + ProximityPrompt (без правок клиента). Коммит `fad72db`, задеплоено (`Successful update operation`, health ok). Проверка: `smoke-roleplay-realnpc.mjs` (Lune compile 5/5 + наличие всех 4 систем) + recognizability 9/9.
- **Реальные модели каталога + ходячие NPC (фидбэк «нпс блочные, я дал ассеты; машины стоят; скучно; нпс должны ходить»)**: roleplay-NPC грузят НАСТОЯЩИЕ модели каталога через `InsertService:LoadAsset` (по верифиц. ID юзера: Tralalero/Tung/Bananita/Mr-Host, иначе round-robin из тематического пака — переиспользую `themeAssetPacks.ts` параллельной сессии 418), strip-scripts+anchor+`ScaleTo`+drop-Humanoid, pcall-fallback на композит/гуманоида. **Ходят по городу** (`wanderModel` — server PivotTo к случайным точкам, лицом по ходу) + **6 ambient-прохожих** оживляют площадь. Машины — видимый «Drive» (ProximityPrompt→`seat:Sit`, только вблизи). Коммит `4aaebeb`, задеплоено (`Successful update operation`, health ok). Проверка: `smoke-roleplay-realnpc.mjs` (Lune luau.compile 5/5 + проводка asset ID), recognizability 9/9 без регрессий. NB: LoadAsset работает в Studio-плейтесте (generate→.rbxl→play); в опубликованном месте ограничен → fallback. **Комплементарно витрине на подиумах (cc34428/5060f64) параллельной сессии: там статичный декор, тут живые ходячие NPC.**
- **Статус**: ✅ закоммичено ... + `00ead40` + `ec5dfff` (apps/functions/src, атомарно) + задеплоено (`api(us-central1) Successful`, health ok). 9/9 жанров в проде. В проде. Push в origin — отдельной командой (§0.5 п.7). NB: оба ранних юзер-скрина («99 Nights = луг», «Lava City = день/сосны») были с генераций ДО соответствующих деплоев — перегенерация даёт night+костёр / lava+город+закат. **Дальше**: (1) Meshy 3D hero-меш за QA для roleplay (платный async — отдельный заход); (2) сигнатурные объекты roleplay; (3) остальные жанры (TD ночные враги и т.д.).
- **Дальше (опц.)**: тематические структуры/реквизит для не-town жанров (TD-башни/racing-трасса под тему); 3D-меш hero-ассеты за QA-гейтом.

### 🎨 [Тёмные темы (Обсидиан/Титан/Неон) + Zero-Read хаптик на готовность .fbx/.rbxm] (2026-06-03, сессия 413)
- **Задача** (пользователь): (1) переключатель тёмной темы в настройках — Обсидиан, Титан, Неон; (2) [UX] лёгкая тактильная отдача на успешное завершение генерации .fbx/.rbxm для закрепления «Zero-Read» паттерна.
- **Тема — архитектура**: приложение исторически светлое-only (`RootView` форсил `.preferredColorScheme(.light)`), цвета — статичные `static let` в [Colors.swift](apps/ios/AIGoldRoblox/DesignSystem/Colors.swift), используются в 50+ файлах. Менять call-sites нереально → сменили **источник**: семантические токены стали `static var { ThemePalette.current.* }`. Добавлены `AppTheme` (light/obsidian/titan/neon), `ThemePalette` (+4 пресета; `.light` байт-в-байт прежние значения → дефолт не меняет вид), `ThemeManager: ObservableObject` (persist `appTheme`). Прокинут `@EnvironmentObject` в [AIGoldRobloxApp.swift](apps/ios/AIGoldRoblox/App/AIGoldRobloxApp.swift)+[RootView.swift](apps/ios/AIGoldRoblox/App/RootView.swift); смена темы → `.id` на `MainTabView` (чистый ре-рендер; выбранный таб в `AppState` переживает) + `.preferredColorScheme(theme.colorScheme)`. UI-переключатель — карточка «Внешний вид» со свотч-превью в [ProfileView.swift](apps/ios/AIGoldRoblox/Features/Profile/ProfileView.swift).
- **Хаптик**: `ExportView` открывается по ручному тапу (не «завершение») → хук поставлен в [GenerationPreviewView.swift](apps/ios/AIGoldRoblox/Features/Generation/GenerationPreviewView.swift) — `.success` хаптик однократно при первом появлении результата с rbxm/fbx (gate `onExportRBXM/FBX != nil`, `.onChange` ловит concept→completed). Файл чистый, не parallel-dirty.
- **Важно про pbxproj**: проект НЕ использует synchronized file-system groups (явные file-references) → новые .swift не попадают в сборку без правки shared-pbxproj. Чтобы не плодить риск конфликта под параллельными сессиями — весь код влит в уже-референсные файлы (тема → `Colors.swift`, хаптик — inline). Новых файлов нет.
- **Проверка**: сборка в отдельный `/tmp/aigold-claude-build` (Xcode юзера открыт — основной build.db lock, §0.7).
- **Git**: НЕ закоммичено (§0.5 — правки в основной папке/main, Xcode видит для локального теста). RootView/Profile, и др. — parallel-dirty; мои hunk'и точечные. Commit/push — отдельной командой.
- **Известные ограничения**: ~7 `Color.white`+11 `.black` жёстко зашитых мест (Auth/CursedUGC/Export/Profile/Onboarding) в тёмных темах могут смотреться неидеально — follow-up по экранам. Смена темы перезагружает стейт навигации внутри табов (trade-off ради полной ре-темизации).
- **Статус**: ✅ ждёт rebuild в Xcode.

### 🛑 [Краш-флоу: убрана плашка «Ready to export» поверх connection error] (2026-06-03, сессия 408)
- **Задача** (маркетинг): на видео «1-Click Outfit Generator» при «Connection error. I stopped generation before export…» прямо над ошибкой висела плашка «Ready to export» и пускала пользователя дальше по мёртвому сценарию (таймкод 00:31).
- **Root cause**: в одиночном outfit-флоу background job не создаётся (`persistLiveGenerationStatus` апсёртит только терминальные статусы), поэтому статус-баннер `ChatView.generationStatusSummary` идёт через `inferredBackgroundGenerationSnapshot()` ([ChatStore.swift](apps/ios/AIGoldRoblox/Features/Chat/ChatStore.swift):2441). Тот сканирует сообщения newest→oldest; failed-проверка искала `"stopped before export"`, но реальный текст — «stopped **generation** before export», а `"connection error"` в списке не было → новейшее (error) сообщение пропускалось, и баннер брал старое «ready/preview» сообщение → «Ready to export».
- **Фикс** ([ChatStore.swift](apps/ios/AIGoldRoblox/Features/Chat/ChatStore.swift) `inferredBackgroundGenerationSnapshot`, ветка failed): добавлены failure-only ключи `"connection error"`, `"session expired"`, `"fake result"`, `"temporarily unavailable"`. Теперь сообщение об ошибке классифицируется как `failed` → баннер «Нужен фикс / Needs fix» + действие «Повторить / Retry», плашка «Ready to export» не показывается. Check #2 (ready) идёт раньше #3, поэтому легитимные «ready» сообщения не задеты.
- **Проверка**: `swiftc -parse` без синтаксических ошибок (полный build — Xcode пользователя открыт, §0.7 build.db lock).
- **Известное ограничение**: сценарий «успешная генерация → Regenerate → connection error» оставляет отдельный старый completed background job (ветка 3 баннера) — для него нужен отдельный reconcile в `stopGenerationFlowAfterError`; для отчётного 1-click сценария фикс полный.
- **Git**: `ChatStore.swift` parallel-dirty (чужой WIP в том же файле), мой hunk НЕзакоммичен — ждёт rebuild в Xcode (§0.7). Push — отдельной командой (§0.5 п.7).
- **Статус**: ✅ ждёт rebuild в Xcode.
...
- **Умное интервью (вариант B пользователя — фидбэк «не оч умное интервью», бэкенд)** ([promptCatalog.ts](apps/functions/src/promptCatalog.ts) `buildInterviewStateInstruction`, ветка `turn === 0`): добавлен guard против переспрашивания. Root cause: turn-0 инструкция форсила базовый вопрос («какой тип пропса?») даже когда первое сообщение — детальный пресет → интервью выглядело тупым. Теперь: если первый ввод уже отвечает на дефолтный вопрос — НЕ переспрашивать, подтвердить и задать единственный полезный НЕотвеченный вопрос (или для low-stakes предложить разумный дефолт с возможностью изменить). Следует прецеденту clothing-hint (`:5343`). Чистый backend (LLM читает текст → rebuild НЕ нужен), эффект на ВСЕ интервью-категории. Не трогал «Never generate on first message» / `maxTurns` / prompt-body.
- **Проверка**: iOS — `swiftc -parse` exit 0 (build в Xcode пользователя, §0.7 `build.db` lock). Backend — `npm run build --workspace apps/functions` ✅ tsc clean; deploy `api(us-central1) Successful`; `/api/health → {"ok":true}`.
- **Статус**: ✅ закоммичено + (бэкенд) задеплоено. Коммиты: `31d07a3` (пресеты — `ChatPresets.swift` целиком + это PROGRESS-entry + ТОЛЬКО hunk `welcomePresets` из `ChatStore.swift`, хирургически §0.7); `7ef50dd` (`promptCatalog.ts`, интервью-фикс, задеплоен §0.6). Остальные 11 hunk'ов ChatStore.swift + файлы параллельных сессий 391/396/399 — нетронуты в working tree. Push в origin — отдельной командой пользователя (§0.5 п.7). iOS-часть (пресеты + видимость) требует rebuild в Xcode; бэкенд-фикс уже живой.
...
- **Что сделано** (5 слоёв, как у rpg/horror/pvp):
  - [gameTemplates.ts](apps/functions/src/gameTemplates.ts): `buildTowerDefenseScript` — server-authoritative TD-игра целиком из кода: 4 темы (meadow/desert/candy/scifi), 6-точечный S-path, 3 типа башен (Cannon/Sniper/Splash) с апгрейдом через ProximityPrompt, enemy lerp-движение по waypoints (Heartbeat, без Humanoid pathfinding), base HP, cash-экономика + leaderstats, DataStore «Best Wave», авто-рестарт session-loop, boss каждый 5-й wave, клиентский HUD LocalScript. + поля `GameTemplateParams`, dispatch, showcase-case.
  - [index.ts](apps/functions/src/index.ts): subcategory-ветка `tower_defense` (парсит GDD → `buildGameplayScript`) + добавлен в `RUNTIME_PLAYABLE_SUBCATEGORIES`.
  - [promptCatalog.ts](apps/functions/src/promptCatalog.ts) + [types.ts](apps/functions/src/types.ts): `smartInterviewTowerDefense` + `generateTowerDefenseGdd`, 9 точек intent-роутинга, registry-запись приведена к реальному билдеру, `PromptIntent` += `tower_defense_interview`/`tower_defense_generation`.
  - iOS: [ForgeView.swift](apps/ios/AIGoldRoblox/Features/Forge/ForgeView.swift) picker-опция (id `tower_defense`), [ChatPresets.swift](apps/ios/AIGoldRoblox/Features/Chat/ChatPresets.swift) 4 пресета + switch-case, [ChatStore.swift](apps/ios/AIGoldRoblox/Features/Chat/ChatStore.swift) intent-маппинг ×2 (критично) + welcome + starter-replies + helper + `gameSubcategories` Set + `briefRuValueMap`, [ChatView.swift](apps/ios/AIGoldRoblox/Features/Chat/ChatView.swift) nav-title, [AIWorkspaceAPI.swift](apps/ios/AIGoldRoblox/Core/API/AIWorkspaceAPI.swift) genre-icon.
- **Проверка**: backend `npm run build --workspace apps/functions` ✅ exit 0 (tsc чисто); iOS `xcodebuild ... -derivedDataPath /tmp/aigold-claude-399 build` → **`** BUILD SUCCEEDED **`** (изолированный DerivedData, Xcode пользователя открыт — §0.7). Детали — `cursor/changelog-399.md` (Задача 2).
- **Commit/Deploy**: commit **`c9641a1`** (атомарно 4 файла `apps/functions/src/*`) → `bash scripts/safe-deploy-functions.sh` ✅ → `functions[api(us-central1)] Successful update operation` → `Deploy complete!` → health `/api/health` `{"ok":true,...}`. Фича в проде (`https://api-z4yzt6dhjq-uc.a.run.app`).
- **Статус**: Tower Defense готов и задеплоен. **iOS-файлы не закоммичены** (ChatStore/ForgeView parallel-dirty — нужен partial-stage; push по §0.5 п.7 по отдельной команде). Исправлены ложные `[x]` в `docs/IMPLEMENTATION_STATUS.md` (готовы только PvP + Tower Defense; остальные 8 жанров — `[ ]`).
- **Дальше**: остальные 8 жанров тем же 5-слойным паттерном, по очереди с проверкой сборки.

### ✨ [Voice-to-Aura] Генерация аур стала prompt-driven — у каждого описания своя точная аура (2026-05-29, сессия 398)
- **Задача**: пользователь — ауры везде одинаковые и неточные; по каждому описанию/промту должна быть своя классная аура. Сначала ресерч.
- **Root cause**: pipeline был **style-preset-driven** — палитра/текстура/движение зашиты в `AuraStyle.luaPaletteHint`, а бриф пользователя шёл крошечным хвостом → два разных описания с одним style давали почти одинаковый промт LLM. Плюс Lua-правило «if unsure use sparkle» → всегда одна текстура; `FALLBACK_LUA` всегда фиолетовый.
- **Ресерч**: best-practice Roblox VFX = несколько слоёв эмиттеров (core+ambient+accent) + ColorSequence-градиент, бюджет ≤500 частиц; офиц. docs не дают список assetId, Toolbox-ID рискованны (модерация) → цвет/движение как безопасные главные рычаги, текстуры только из 3 уже задеплоенных ID. (Детали и ссылки — `cursor/changelog-398.md`.)
- **Решение** (только в чистых файлах, параллельные не тронуты):
  - [auraStyles.ts](apps/functions/src/data/auraStyles.ts): новый `resolveAuraVisuals` — из брифа (EN+RU) извлекает названные цвета (15 + rainbow), движение (rise/swirl/burst/fall/electric/flow/pulse) и текстуру (из `AURA_TEXTURES`). RU-стемы заякорены, чтобы «прекрасный»≠red и т.п. Билдеры `buildAuraImagePrompt`/`buildAuraLuaPrompt` переписаны: бриф = PRIMARY spec, цвета OVERRIDE палитру (точные `Color3.fromRGB`), движение → свойства эмиттера, слоистая композиция + PointLight. Лимит Lua 80→130 строк, combined Rate ≤250 сохранён, набор текстур жёстко ограничен 3 ID (безопаснее прежнего).
  - [auraGenerator.ts](apps/functions/src/auraGenerator.ts): `FALLBACK_LUA` → `buildFallbackLua(rgb, textureId)` — даже fallback берёт цвет+текстуру из промта.
- **Проверка**: `npx tsc --noEmit` (apps/functions) → exit 0; юнит-тест лексикона (tsx) 17/17 (вкл. RU-guards и rainbow радуга/радужная).
- **Commit/Deploy**: коммит **`a1771f4`** (атомарно 2 файла) → `bash scripts/safe-deploy-functions.sh` ✅ → `functions[api(us-central1)] Successful update operation` → `/api/health` ok (`https://api-z4yzt6dhjq-uc.a.run.app`). Фикс в проде. **Push в origin не делал** (по §0.5 — по отдельной команде).
- **Как проверить**: сгенерировать ауру в приложении с разными описаниями (напр. «золотой огонь вверх» vs «ледяная синяя воронка») — должны отличаться цветом, движением и текстурой, а не быть одинаковыми.
...
### ✅ [Fitting Room] «Твой аватар» (Option A, server-composited) + фикс render-403 от битых curated-ассетов (2026-05-29, сессия 394)
- **Фича (Option A)**: режим «Твой аватар» показывает РЕАЛЬНУЮ полную одежду, запечённую на теле самого юзера, через server-side `POST /v1/avatar/render` (а не клиентское навешивание аксессуаров). backend `renderOutfitOnUser3D` + поле `render3dUser` в fitting-room doc (`6db568e`); iOS декодит `render3dUser`, в режиме «Твой аватар» грузит персонализированный composited OBJ с mode-specific cache-key (`38a1401`).
- **Баг-репорт пользователя** (2 скрина): «Твой аватар» → HTTP 502; Y2K-принцесса манекен R15 БЕЗ одежды.
- **Root cause (оба бага = один)**: `POST avatar.roblox.com/v1/avatar/render` отдаёт **403** если в списке есть НЕносимый ассет — `"...not authorized for operation GetModerationStatus and asset type Place."`. ОДИН плохой ассет травит ВЕСЬ рендер → render=null → нет `render3d`/`render3dUser` → манекен фолбэк на тело без одежды, «Твой аватар» фолбэк на 502-ящий `.realUser`. Источник — битый курированный пул: все 5 id мусор (probe через `develop.roblox.com/v1/assets`: Lua 5 / Place 9 / Model 10 = НЕносимые; + Hat и TShirt «hmm» мис-подписанные). Y2K тащил именно Place → гарантированный 403.
- **Решение (commit `ac0774f`, 2 слоя)**:
  - **Layer A** `robloxAvatar3D.ts`: `filterRenderableAssetIds` batch-резолвит типы через `develop.roblox.com/v1/assets` и выкидывает НЕносимые ДО POST (allowlist носимых typeId, fail-open на ошибку lookup'а); вкручен в `renderOutfit3D` + `renderOutfitOnUser3D`.
  - **Layer B** `outfitAesthetics.ts`: опустошены 5 битых curated-cores (SIGMA/BADDIE/Y2K/GOTH/CYBER) — теперь все эстетики на live catalog search, как 4 остальные.
- **Проверка**: build ✅; deploy `safe-deploy-functions.sh` ✅ `Successful update operation` + `/api/health` ok:true. Live A/B против реального эндпоинта: render([Hat, **Place**]) → **403** (баг воспроизведён); render([Hat]) → **200/Pending** (после фильтра 403 нет). Root cause доказан на проде.
- **Что проверить в приложении**: сгенерировать **НОВЫЙ** Y2K-образ (render3d/render3dUser пишутся только в доки после deploy) → манекен с рубашкой+штанами; «Твой аватар» (с подключённым Roblox-username) без 502, фит запечён на теле юзера.
...
- **Решение** (2 коммита):
  - **backend** `outfitAssembler.ts` (commit `b93aa76`): helper `filterClassicClothing` в точке сбора пула `searchSlotCandidates` (питает и главный assemble, и Swap-grid `getSlotAlternatives`). shirt/jacket → только classic Shirt (assetType 11), pants → только classic Pants (12). Fallback на исходный пул, если classic нет. tshirt намеренно НЕ в карте — мелкий принт отклонён. Проверено по `outfitCatalogPool.json`: у всех 9 эстетик ≥1 classic shirt и ≥3 classic pants → слот никогда не пустеет, регрессии «слот пропал» нет.
  - **iOS** `RobloxAvatar3DViewer.swift` (commit `1e39384`): `applyTextureToBody` собирает части тела РЕКУРСИВНО — старый non-recursive `avatarRoot.childNodes` находил 0 geometry, если `.scn` (грузится раньше `.obj`) вкладывает Rig-части под контейнер-нод (Xcode OBJ→SCN так делает) → текстура молча не накладывалась. Теперь обход любой глубины (пропуская `AccessoryAttachment-*` subtree'и, чтобы шляпа/волосы не красились одеждой); upper/lower Y считается в системе координат AvatarRoot через `convertPosition` (корректно при любой вложенности).
- **Проверка**: backend `npm run build --workspace apps/functions` ✅ (tsc clean); iOS `swiftc -parse` ✅ + полный `xcodebuild ... -derivedDataPath /private/tmp/aigold-clothing-394 ... build` ✅ `BUILD SUCCEEDED` (отдельный DerivedData — Xcode пользователя открыт, §0.7); temp DerivedData удалён.
- **Commit/Deploy**: backend `b93aa76` + iOS `1e39384` закоммичены; backend задеплоен `functions:api` через `scripts/safe-deploy-functions.sh` ✅ `Successful update operation` (Function URL `https://api-z4yzt6dhjq-uc.a.run.app`); health check `/api/health` → `{"ok":true,"service":"ai-roblox-gold-firebase-api","region":"us-central1"}` ✅. iOS-коммит ждёт сборки в Xcode пользователя.
- **Что проверить в Xcode**: Zero-Robux Fitting Room → сгенерировать любой образ (напр. Y2K) → в 3D dress-up room на манекене должна появиться ПОЛНАЯ рубашка (торс+руки) и/или штаны — не пустое тело и не мелкий принт на груди.
...
- **Deploy**: `bash scripts/safe-deploy-functions.sh` ✅; `/api/health` ✅; Cloud Run live revision `api-00973-qug`, traffic `100%`. Commit `2291dcb` (атомарно через private index по §0.7, не подобрал чужие dirty hunks). Plus follow-up audit-fix: убрал `hasLlmScene` short-circuit в 3 местах car-QA — теперь baseline-силуэт всегда валидируется (раньше любой LLM-accent скипал проверки).
- **Follow-up (тот же день, сессия 373)**: пользователь скинул две референсных rbxm (машина.rbxm = 61 MeshPart, тесла.rbxm = 179 MeshPart) — стало ясно что нужны меши, не сборка из примитивов. Переключил vehicle mesh stage с **Tripo v2.5 → Meshy 6 via fal.ai** (`fal-ai/meshy/v6/text-to-3d`, $0.80/генерация). Используется уже существующая `runMeshy()` функция в `providers.ts:3235` (имеет vehicle-specific negative_prompt + 3-attempt retry на downstream_service_error). `runTripoVehicleText` оставлен как dormant код для rollback'а. Fallback chain теперь: Meshy → procedural baseline + LLM accents. Tripo полностью убран из активного пути. Build clean, все 7 smoke PASS.
...
- **Решение** (3 layer):
  - **Layer 1 — визуальная дифференциация в `apps/functions/src/robloxWorker.ts`** (family-sedan ветка `addVehicleBodyShell`): `FamilyCarRoofPanel` теперь `accent` Metal (контрастный тёмный roof) + `FamilyCarRoofEdgeTrim`; windshield/rear/side windows transparency 0.38-0.42 (вместо 0.16-0.18); SteeringWheelVisible cylinder увеличен до диаметра 1.30 stud, `sedanSteeringY=rootY+h*0.72`, `sedanSteeringZ=-l*0.18` — поднят и сдвинут вглубь cabin, виден через прозрачное лобовое; новые accent-канты `HoodToWindshieldTrim`/`CargoToRearGlassTrim`/`Front+RearBeltLineSegment`; `repairBoost` усилен `RoofSpoilerAccent`/`HoodLouvre*`/`RockerStripe*`.
  - **Layer 2 — детерминированные визуальные heuristics в `apps/functions/src/index.ts`** (`computeVehicleVisualFacts`, `vehicleManifestFacts`, `deterministicVehicleReview`): 4 новых факта — `monochromeBodyShare` (≥0.8 → `monochrome_silhouette`), `bodyVerticalLayerRange` (<1.2 → `flat_body_layers`), `glassToBodyAreaRatio` (<0.08 → `low_glass_area`), `steeringWheelVisibleThroughWindshield` (false → `steering_wheel_hidden`). Каждый flag → reject + repairBoost retry. HIDDEN_RX исключает ChassisRoot/Constraint helpers из статистики.
  - **Layer 3 — ASCII side-silhouette `renderVehicleSideSilhouetteAscii`** в `apps/functions/src/index.ts`: детерминированный 12×24 grid side-view проекции манифеста (R=roof, C=cabin, G=glass, W=wheel, S=steering, B=body, .=empty). Грид кладётся в `manifestDigest.sideSilhouetteAscii`, system prompt LLM-критика учит читать его и reject'ить «slab silhouette» (одна полоса B без C/R/G/S). Без headless рендера PNG, latency 0, cost 0.
- **Проверка**: `npm run build --workspace apps/functions` ✅; 3 smoke'а PASS:
...
- **Deploy**: production `api` deployed successfully; `/api/health` ✅; Cloud Run latest ready/live revision `api-00929-ler`, traffic `100%`.
...
- **Deploy**: production `api` deployed successfully; `/api/health` ✅; Cloud Run latest ready/live revision `api-00927-fev`, traffic `100%`.
...
- **[Furniture AI-Mesh size & preview — FIXED]** (сессия 402): исправлены два бага mesh-режима. (1) Запечённый `AIMeshBody` MeshPart теперь масштабируется одним коэффициентом под footprint типа (`handleSize`), сохраняя пропорции — раньше брался сырой inner-MeshPart bbox (десятки studs) → prop был огромным на карте. (2) iOS-превью mesh-мебели показывает concept-рендер (`AsyncImage`) вместо пустого SceneKit-бокса — URL едет внутри `furnitureSpecJSON`. Backend задеплоен (`api(us-central1)`, commits `14a1bf4` + `9bafde5`); iOS-изменения (`GenerationPreviewView`, `BlockyFurniture3DSceneView`) видны после пересборки приложения в Xcode. Старые furniture `.rbxm` не переписываются — для проверки нужен fresh regenerate/export.
- **[Cursed UGC — честный .fbx экспорт + дисклеймер — DEPLOYED]** (сессия 406): правка маркетинга. (1) Backend строит настоящий текстурированный `.fbx` из Blender-optimized GLB через воркер `/convert-to-fbx` (`glb_to_fbx.py`: `embed_textures=True`, текстуры ≤1024px — фикс «Material_0 Upload failed»); `fbxUrl` → `CursedUGCResult` → артефакт + payload (`viralChatDispatch.ts`) → iOS (`CursedUGCResponse`). (2) iOS кнопка экспорта: `.fbx` (зелёный headline, формат для публикации UGC) → `.rbxm` → `.glb` (fallback). (3) Дисклеймер «NOT a real Roblox UGC item / not for upload» заменён на честный («экспортни .fbx и импортни в Studio чтобы опубликовать как свой UGC; цена/статы — стилизованное превью»). **Backend задеплоен** (commit `c40acbe`, `api(us-central1)` Successful update, `/api/health` ok). **ОСТАВШИЕСЯ ГЕЙТЫ**: (a) `.fbx` текстурный только если в Cloud Run воркере задеплоен session-083 `glb_to_fbx.py` с texture-embed (Docker redeploy воркера НЕ подтверждён — Worker-service known-issue выше); иначе graceful fallback на `.rbxm`/`.glb`. (b) iOS пересборка в Xcode (кнопка `.fbx` + новый текст; iOS-файлы НЕ закоммичены — не build-verified). (c) live-прогон одного cursed-айтема для подтверждения, что `.fbx` реально открывается текстурированным в Studio.
- **[Result preview sheet flicker — FIXED]** (сессия 412): правка маркетинга. При открытии результата генерации/превью по пушу или по плашке-баннеру экран результатов мигал (открывался/закрывался несколько раз). Root cause: `.sheet(item: $activePreview)` ре-кеился, потому что поток открытия вызывает `restoreGenerationJob(openWhenReady:)` несколько раз (`hydrateLatestJobFromRemoteThread` + `restorePreviewFromJob`, гард де-дупа обходится при `openWhenReady`), а `makePreviewPayload` отдавал новый `UUID().uuidString` id на каждый вызов. Фикс: в `openBackgroundGeneration` id результата привязан к job (`preview.withId("background-result-<job.id>")`), повторные присваивания того же job — no-op для sheet. Тот же паттерн, что уже используется для foreground (`.withId("pipeline-live")`). iOS-файл `ChatStore.swift` НЕ закоммичен (был dirty от параллельных сессий) и НЕ build-verified — нужна пересборка в Xcode + ручной прогон открытия по пушу и по баннеру. Возможная вторичная причина (мигание внешнего `fullScreenCover`/`launchConfig` на пуш-пути) не трогалась — см. changelog-412.
