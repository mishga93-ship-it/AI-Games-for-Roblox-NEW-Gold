...
export type ArtifactType = 'lua' | 'text' | 'gdd' | 'json' | 'project_bundle' | 'archive' | 'rbxm' | 'rbxl' | 'png' | 'fbx' | 'glb' | 'obj' | 'usdz' | 'audio' | 'mp4' | 'gif';
...
export type ContentCategory = 'script' | 'game_system' | 'ui' | 'character' | 'weapon' | 'vehicle' | 'building' | 'map' | 'pet' | 'ugc_clothing' | 'ugc_accessory' | 'avatar_body' | 'furniture_prop' | 'item_tool' | 'decal_texture' | 'plugin' | 'animation' | 'audio' | 'effect' | 'other';
export type ProjectKind = 'game' | 'content' | 'fix' | 'clone' | 'ugc' | 'analyze';
...
export type GenerationStageId = 'concept_image' | 'clothing_texture' | 'mesh_3d' | 'convert_fbx' | 'upload_roblox' | 'mesh_optimized' | 'rig_r15' | 'generate_cages' | 'package_accessory' | 'export_model' | 'export_rbxm' | 'generate_keyframes' | 'generate_decal_image' | 'generate_vehicle_scripts' | 'generate_vehicle_mesh' | 'generate_vehicle_scene' | 'quality_review';
...
export interface RobloxBuildScript {
    ...
    container: 'ServerScriptService' | 'ReplicatedStorage' | 'StarterPlayerScripts' | 'StarterGui' | 'Workspace';
    source: string;
}
...
export type ModerationEntityType = 'chat' | 'artifact' | 'project' | 'comment' | 'profile' | 'report' | 'voice' | 'attachment';
export type ModerationCaseStatus = 'open' | 'under_review' | 'resolved' | 'appealed' | 'dismissed';
...
export type BadgeId = 'top_creator' | 'game_developer' | 'script_master' | '1000_downloads' | '100_likes' | 'rising_star' | 'first_publish' | 'community_helper' | 'ugc_designer' | 'prolific_publisher';
