# AI Games for Roblox — Карта Релизов 3, 2 и 1

**Дата:** Июнь 2026

> Структура: сверху — **Релиз 3** (что реализовано в текущем релизе), ниже — объединённая карта **предыдущих релизов 2 и 1**.
...
**Релиз 3 = UGC-Лихорадка, Понты и Roleplay.**

Этот релиз давит на социальное доминирование, хайп вокруг субкультур и визуальный статус. Главная идея: дать пользователю фабрику UGC-контента (одежда, аксессуары, ауры, проклятые ассеты, fake-лимитки) + оставшиеся жанры игр + инструменты редактирования и ремикса чужого кода, замкнув цикл «сгенерировал → примерил → опубликовал на Marketplace». Большинство killer-фич ложатся на Clothing и Accessories.

---

## I. Killer Features Релиза 3

| Killer feature | Статус | Зачем нужна |
|---|---|---|
| "Fake Headless & Korblox" AI Crafter | Реализовано / RC | Обещание дорогих лимиток (Headless/Korblox) за ноль робуксов = космический CTR в App Store |
| "Baddie / Sigma" 1-Click Outfit Generator | Реализовано / RC | Сеты в один клик под конкретную эстетику (9 эстетик: sigma/baddie/y2k/goth/cyber/...). Бьём в актуальные тренды |
| Giant & Cursed UGC Modeler | Реализовано / RC | Проклятые/уродливые/гигантские ассеты — топ маркетплейса (7 категорий) |
| Voice-to-Aura (Particle Engine) | Реализовано / RC | Голос → анимированная аура-аксессуар (8 стилей); визуально продаёт приложение за доли секунды |
| "Zero-Robux" UGC Fitting Room | Реализовано / RC | Сгенерировал → сразу примерил на свой ник (3D-рендер + превью на аватаре). Мощный триггер для покупки IAP |
| Voice-Controlled Survival Disaster Spawner | Реализовано / RC | Голосом спавнишь катастрофы (4 режима: funny/horror/meme/sigma); стыкуется с жанром Survival |

**Итог:** все 6 killer-фич реализованы (iOS + backend, вход через Forge). Fake Headless & Korblox шипается через **Avatar Glow-Up** chat-flow: тайл `fake_limited` → ChatView-интервью → богатый `GlowupResultView` через `GlowupChatBridge` (backend `/api/glowup/generate`; legacy `/api/fake-limited/recipe` ещё подключён как fallback). Файл `Features/FakeLimited/FakeLimitedRecipeCard.swift` — осиротевший косметический «Variant 3» старого пути: уже в Xcode-проекте, но под `#if false`; на работу фичи не влияет (можно удалить).

---

## II. Генерация нового контента

| Категория | Что реализовано во третьем релизе | Статус | Формат |
|---|---|---|---|
| Furniture & Props | Столы, стулья, лампы, декор, вывески, растения для заполнения зданий и карт. Детерминированный hybrid-skeleton (chair/table/shelf/bed/lamp/plant/sign) + LLM-акценты, blocky-превью + RBXM. | Реализовано / RC | `.rbxm` (один проп за генерацию; «pack»-чип = тематический single-prop) |
| Clothing & Outfits | Classic: T-Shirt/Shirt/Pants как PNG-шаблоны (585×559 wrap). Layered: Jackets/Sweaters/Dresses как 3D (.fbx через cage-service + WrapLayer). Загрузка на Marketplace per-user OAuth + Vision-модерация. | Реализовано / RC | PNG-шаблон + `.rbxm` / `.fbx` |
| Vehicles | Машины/мотоциклы/лодки/самолёты/танки: drive-скрипт + пассажиры + звуки + физика. Активный путь — 13 curated Roblox-шаблонов; модульный «brainrot»-слот-генератор (body/head/wheels/engine/effects/sound) — code-side (Phase A). | Реализовано / RC | `.rbxm` + scripts |
| Pets как отдельные ассеты | Модель + follow-анимация + левелинг + редкость + стихия. 3D-путь (Tripo, 3 стадии эволюции, rig) и blocky-путь (Motor6D). Отдельно от pet-system скрипта симулятора. | Реализовано / RC | `.rbxm` + scripts |

---

## III. Генерация игр — оставшиеся жанры

Все 9 жанров прошиты через единый `buildPlayableWithQa`: per-preset visual-spec (`deriveGameVisualSpec`) + авто-QA (`reviewGameQuality`) с self-heal-перегенерацией. 9/9 в проде.

| Жанр | Статус | Что добавлено |
|---|---|---|
| Roleplay / Town | Реализовано / RC | Тематические здания, живые работы (clock-in → доставка), свой дом (claim + мебель), монеты-квест, автосалон + банкомат, ходячие R15-NPC, реальные модели каталога |
| Tower Defense | Реализовано / RC | Враги по вайбам (реальные мем-лица), боссы, ночные атмосферы + костёр |
| Racing | Реализовано / RC | Vibe-контент: лава/город/пальмы, тематический спавн-баннер |
| Parkour | Реализовано / RC | Палитра + небо + тематический landmark |
| Story Game | Реализовано / RC | Катсцены, ветвление/выборы с последствиями, 3 концовки, NPC-диалоги, головоломка |
| Mini-games Hub | Реализовано / RC | Тематический спавн-баннер + палитра |
| Survival | Реализовано / RC | Преследователи берут цвет/имя/лицо из ростера вайба |
| Fighting | Реализовано / RC | PvP-ринг + тематический landmark |
| Custom | Реализовано / RC | Тематический спавн-баннер + палитра/атмосфера |

---

## IV. Инструменты: фикс, редактирование, ремикс

| Функция | Статус | Что делает |
|---|---|---|
| Voice-to-Fix (AI Luau Doctor) | Частично | Текстовый `scriptDoctor` готов (разбор причины + безопасный фикс, красный → зелёный код). Upload `.lua` + голосовое описание проблемы — не закрыт |
| Редактирование существующего контента | Code-side готово / нужна QA | Upload `.lua` → анализ → правки голосом/текстом → переписывание. Роуты `/api/attachments/ingest`, `/analysis`, `/edit-preview`, `/apply`. Upload описания игры → предложения улучшений |
| AI Remix / Fork | Реализовано / нужна QA | «Remix with AI» по чужой публикации → вариация, оригинальный автор указан (HomeView/CatalogView + remix-intent; Outfit remix-режимы) |

---

## V. Экспорт UGC-контента

| Формат / функция | Статус | Что делает |
|---|---|---|
| PNG-шаблоны (classic clothing) | Реализовано | Шаблоны Shirt/Pants для загрузки на create.roblox.com |
| .fbx-гайд (layered clothing / аксессуары) | Реализовано | Корректный Accessory-flow (импорт `.fbx` → WrapLayer), а не full-avatar rigging |
| Инструкция загрузки на Creator Hub | Реализовано | `ExportView` + `MarketplaceHandoffView` со ссылками create.roblox.com |

---

## VI. Сообщество и модерация

| Функция | Статус | Что делает |
|---|---|---|
| Weekly Challenges | Реализовано / RC | Еженедельные конкурсы, голосование сообщества, призы-токены, фичеринг победителей (`Challenge` статусы active/voting/history, scheduled-tally, `ChallengesView`) |
| Модерация публикаций (Vision AI) | Реализовано / RC | Vision-AI по скриншотам (`runGeminiVision`, Gemini 2.5 Flash) + проверка скриптов на вредоносный код (`moderateLuaSource`/`scriptSafety`) перед публикацией/загрузкой |
| Модерация комментариев (AI < 500мс) | Реализовано | `runModerationPipeline` на тексте комментария перед сохранением; серая зона → review |

---

## Сводка Релиза 3

| Категория | Готово / RC | Code-side / нужна QA | Частично |
|---|---:|---:|---:|
| Killer features | 6 | 0 | 0 |
| Генерация контента | 4 | 0 | 0 |
| Жанры игр | 9 | 0 | 0 |
| Инструменты (фикс/правки/ремикс) | 0 | 2 | 1 |
| Экспорт UGC | 3 | 0 | 0 |
| Сообщество и модерация | 3 | 0 | 0 |

**Открытые хвосты Релиза 3:**

1. **Voice-to-Fix** — есть текстовый `scriptDoctor`; upload `.lua` + голосовой ввод проблемы не закрыт.
2. **Редактирование контента / AI Remix** — code-side готовы, нужна device-QA полного флоу.

> Примечание: `Features/FakeLimited/FakeLimitedRecipeCard.swift` (Variant 3) — мёртвый косметический код старого session-382 пути; фича Fake Headless & Korblox работает через Glowup. Файл уже в Xcode-проекте под `#if false`; его можно удалить, на фичу это не влияет.

---

# Часть B. Предыдущие релизы (2 и 1) — объединённая карта

> Ниже — карты Релиза 2 и Релиза 1 в исходном виде, под общим разделом «предыдущие релизы».
